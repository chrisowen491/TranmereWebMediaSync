...
The SAM CLI installs dependencies defined in `MediaSync/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
$ sam local invoke MediaSync --event events/event.json
```
...
```bash
$ npm run build
$ sam deploy --stack-name MediaSync --capabilities CAPABILITY_IAM --s3-bucket tranmere-web-api
```
...
```bash
$ sam logs -n MediaSyncFunction --stack-name MediaSync --tail
```
...
```bash
$ aws cloudformation delete-stack --stack-name MediaSync
```
